...
function pack() {
    ...
    let wasm = fs.readFileSync('dist/js.wasm');
    lzma.compress(wasm, 8, (result, error) => {
        ...
        fs.writeFileSync('scripts/js.wasm.lzma', new Uint8Array(result));
    }, (percent) => {
...
function unpack() {
    ...
    let cmp = fs.readFileSync('scripts/js.wasm.lzma');
    lzma.decompress(cmp, (result, error) => {
        ...
        fs.writeFileSync('dist/js.wasm', new Uint8Array(result));
    }, (percent) => {
        process.stdout.write('\r' + (Math.round(percent * 100)) + '%  ');
    });
}
