...
const commentRegExp = cre.global`
    "*/"
`;

const templateRegExp = cre.global`
    lazy-repeat (("\\", any) or any);
    "\`";
`;
...
const interpolationRegExp = cre.global`
    "}"
`;
...
class CRESemanticTokensProvider implements monaco.languages.DocumentSemanticTokensProvider {
    ...
    provideDocumentSemanticTokens(model: monaco.editor.ITextModel, lastResultId: string | null, token: monaco.CancellationToken) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (state === ParsingState.CODE && line.indexOf('`') < 0 && line.indexOf('/*') < 0) continue;
            ...
            do {
                switch (state) {
                    case ParsingState.CODE:
                        codeRegExp.lastIndex = index;
                        match = codeRegExp.exec(line);
                        index = codeRegExp.lastIndex;
                        state = this.parseCode(match);
                        break;
                    case ParsingState.COMMENT:
                        commentRegExp.lastIndex = index;
                        match = commentRegExp.exec(line);
                        index = commentRegExp.lastIndex;
                        if (match) {
                            state = ParsingState.CODE;
                        }
                        break;
                    case ParsingState.TEMPLATE:
                        templateRegExp.lastIndex = index;
                        match = templateRegExp.exec(line);
                        index = templateRegExp.lastIndex;
                        if (match) {
                            state = ParsingState.CODE;
                        }
                        break;
                    case ParsingState.CRE:
                        creRegExp.lastIndex = index;
                        match = creRegExp.exec(line);
                        index = creRegExp.lastIndex;
                        state = this.parseCRE(i, match);
                        break;
                    case ParsingState.INTERPOLATION:
                        interpolationRegExp.lastIndex = index;
                        match = interpolationRegExp.exec(line);
                        index = interpolationRegExp.lastIndex;
                        if (match) {
    ...
    parseCode(match: RegExpExecArray | null): ParsingState {
        if (!match) return ParsingState.CODE;
